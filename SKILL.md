...
| 动作2 讲解 | 100-200 字短讲，**不配图** | 标准讲解 + 配图（信号强制/学员选择双轨） | 标准讲解 + 配图 + 信息图（双轨，可含交互动画） |
...
| 视觉功能 | 全部跳过 | 信号强制/学员选择双轨 | 信号强制/学员选择双轨（全型可用，含交互动画） |
...
- 讲解前必调：`python db.py graph --topic X`（JSON 输出含 `need_visual` 信号：true=强制先画，false/无信号=学员选择制推荐询问）
...
**B. 推荐轨（无 need_visual 信号或 false）**：讲解开始前 AGENT 基于概念性质**主动推荐并询问**：

> "这个概念涉及 {流程/层级/关系对比}，我可以画一张 {图型} 帮助理解，需要吗？也可以指定别的画法。"

- 学员可拒绝（纯文字讲解），也可在会话中**随时主动要求配图**（如"画个图"）——学员主动要求时直接执行
- depth=aware 概念不配图（无论信号）；交互动画属可选增强（depth=apply、level≥3、有可观察动态特征），生成成本高，仍须学员同意后生成
...
2. 自由回忆："不看任何笔记和资料，把你知道的关于 {topic} 的一切说出来。不要求完整准确，片段、猜测、不确定的都算数。"（aware 级概念简化为是非法/单选式回忆）

**回忆后（全部执行）**：
1. 先真诚肯定尝试，再评估（绝不贬低）。
2. 明确指出三件事：说对了什么（学习者往往不知道自己对在哪）、缺了什么（未提及的关键概念）、误解了什么（部分或完全错误的想法——温和但清晰地指出）。
3. **只教缺口**：基于回忆中暴露的缺口和误解教学，引用学习者原话："你提到 X——这部分是对的，我们在这个基础上继续。"
...
**证据产出**：`evidence log --kind recall --outcome accurate|partial|minimal|warm_start --detail "缺口/误解摘要"`
...
3. **配图触发（双轨）**：讲解前查"视觉输出纪律"——need_visual=true 直接先画结构图（不询问）；false/无信号则推荐图型并询问学员，学员同意才配图；学员随时主动要求配图时直接执行。图档按教学材料库约定存档复用。
...
2. **讲解扩容**（替换默认讲解结构，各节禁止一两句带过）：
   ...
   - 精确定义 + 符号约定（每个符号明确含义）
   - 关键定理与性质：标注哪些只要求会用、哪些要求会证
   - 证明思路框架：分步给出，每步标注"为什么这样做"；完整推导留给习题
   - 典型例子与反例：正例说明定义为何这样设计，反例划清概念边界
   - 迷思预警：符号误用、条件遗漏、直觉陷阱各至少 1 个
3. **习题扩容**：检验题之外，每概念配一组分层习题（≥5 题）——
   - 1-2 题基础（直接套定义，练符号熟悉度）
   - 1-2 题标准（教材课后题难度）
   - 1 题变式（改条件/反例构造/证明题）
   - 可加 1 题综合（与前置概念联动）
   - 每题标注：练习方式（**手练**：推导/手算；**电脑练**：numpy/scipy 验证）与考察点
4. **数值验证**：给出可用 numpy/scipy 复现结论的代码片段（如适用），学员可即时运行核对
...
### 动作 4：信心校准（confidence-calibration-check）——v0.4.0 已移除

> **本动作自 v0.4.0 起移除**：每次任务前后要求学员打信心分（0-100）过于频繁，干扰教学节奏。
> 过度自信/低自信改由**行为信号**推断（答错率与表达流畅度的反差、连续卡壳、示例质量），不再向学员索取任何评分。
> db.py 的 `--confidence-before/after` 参数保留但教学流程不再使用（向后兼容）。
...
**讲解时**（动作 2）：配图触发走双轨（need_visual=true 强制先画；false/无信号按学员选择制推荐询问），depth=apply 且 level≥3 的概念可推荐 Mermaid 图。
...
4. ~~**信心自评受习惯性高估/低估影响**~~（v0.4.0 已移除信心打分）：过度自信/低自信改由行为信号推断（答错率、卡壳模式、示例质量），不再向学员索取评分。
...
- **new**: 数理类强化模式（动作 2）——数学/物理/统计等形式化学科的 understand/apply 概念：① 教材定位（经典教材章节锚点，concept add/edge add 按教材章节顺序编排）② 讲解扩容（双直觉、定义+符号约定、定理与证明思路框架、正反例、迷思预警）③ 习题扩容（每概念 ≥5 题分层：基础/标准/变式/综合，标注手练/电脑练与考察点）④ numpy/scipy 数值验证片段
- **兼容**: DB schema、SM-2 逻辑、其余动作协议零改动；动作编号保留（动作 4 位置标记为已移除），存量概念与复习计划不受影响
