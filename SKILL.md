...
**版本策略（V 后三位数字语义，底层约定）**：
- 第一位（x.0.0）：**彻底重构、推倒重来**——推翻现有实现/协议整体重写时才升
- 第二位（0.x.0）：**重大架构调整**——DB schema、教学框架、动作体系增删等结构性变更
- 第三位（0.0.x）：**仅小改动**——交互协议细节、表述、局部规则修订
- 低位数升级时清零：0.3.1 小改 → 0.3.2；结构性调整 → 0.4.0；推倒重构 → 1.0.0
- 同日多次小改合并为一次发布（0.3.1 → 0.3.2），不逐次升级
